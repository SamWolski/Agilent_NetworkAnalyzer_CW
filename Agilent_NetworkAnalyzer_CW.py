...
class Driver(VISA_Driver):
    ...
    def initLogger(self):
        ...
        log_file = 'PXI_VNA_CWext_{:%y%m%d_%H%M%S}.log'.format(datetime.now())
        ...
        logging.basicConfig(filename=log_path, filemode='a',
                            ...
                            datefmt='%y-%m-%d %H:%M:%S')
        self.logger = logging.getLogger('vna_log')
        self.logger.info('Logging initialized to {}'.format(log_path))
    ...
    #                    iTrace = int(lTrace[-1]) + 1
            ...
            elif value == 'Lorentzian': # Lorentzian / segments
                # prepare VNA for segment sweep
                self.writeAndLog(':SENS:SWE:TYPE SEGM') 
                self.writeAndLog('DISP:WIND:TABL SEGM')
            # # if time-domain
            # elif self.getValue('Sweep type') == 'Time':
            elif value == 'Time': # CW time
                self.writeAndLog(':SENS:SWE:TYPE CW')
                # self.writeAndLog(':SENS:SWE:TYPE CW')
                # self.writeAndLog(':SENS:SWE:TYPE FACW')
            else:
                self.askAndLog('*IDN?')
        else:
    ...
    def performGetValue(self, quant, options={}):
        ...
        if quant.name in ('S11 - Enabled', 'S21 - Enabled', 'S12 - Enabled',
            ...
            if quant.name in self.dMeasParam:
                ...
                if bWaitTrace:
                    if bAverage:
                        ...
                        ## Log trigger initiation event
                        self.logger.info('Immediate-mode trigger sent.')
                    ...
                    while (not bDone) and (not self.isStopped()):
                        ...
                        if not bDone:
                            self.wait(0.1)
                    self.logger.info('Trace done or VNA stopped.')
            ...
            ## Log completion of data read operation
            self.logger.info('Trace read operation completed.')
        elif quant.name in ('Wait for new trace',):
            # do nothing, return local value
            value = quant.getValue()
        elif quant.name in ('Sweep type'):
            ## Get string corresponding to sweep type
            sweep_string = self.askAndLog(":SENS:SWE:TYPE?")
            ## Convert to integer in lookup table
            sweep_lookup = {"LIN": 0, "LOG": 1, "SEGM": 2, "CW": 3}
            value = sweep_lookup[sweep_string]
        else:
